...
const Homepage = () => {
  ...
  // NEW: State for featured books from DB
  ...
  useEffect(() => {
    axios
        .get('http://localhost:8080/api/products') // your backend endpoint to list all books
        .then((response) => {
          // For convenience, map `isbn` to `id`, so existing code can still use `book.id`
          const dataWithIds = response.data.map((item) => ({
            ...item,
            id: item.isbn, // keep referencing it as `book.id` in your JSX
          }));
